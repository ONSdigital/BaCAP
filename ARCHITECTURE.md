...
Every file the user can download is **generated on the fly in the browser** from data already
loaded on the page. No file is created on or stored by a server, so **downloads have no persistent
URL**: a downloaded file can't be linked to, bookmarked or fetched again later. To get it again,
the user has to rebuild the profile or dataset and download it again.

| Download                         | Generated by                                                             | Built from                                                  |
| -------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------- |
| Profile XLSX / CSV (`/build`)    | `downloadProfileXLSX()` / `downloadProfileCSV()` in `io/index.js`        | the `tables` returned by `getData()`                        |
| Profile PNG (`/build`)           | `snapdom` inside the `/profile` iframe, triggered by a `png` pym message | the rendered profile page                                   |
| Dataset XLSX / CSV (`/download`) | `downloadDatasetXLSX()` / `downloadDatasetCSV()` in `io/index.js`        | the dataset fetched with `getData()` for the selected areas |
| Area GeoJSON (`/build`)          | `downloadArea()` in `geo/index.js`                                       | the current `activeArea`                                    |

XLSX files are built with `@onsvisual/accessible-xlsx`, and CSVs with `d3-dsv` (with a UTF-8 byte
order mark so Excel reads accented characters correctly). In each case the file is wrapped in a
`Blob`, and `download()` in `io/index.js` gives it a temporary object URL (`blob:…`) and clicks a
hidden `<a download>` link. That object URL works only in the current page, so it can't be shared.
The PNG is saved the same way by `snapdom`'s own download function, inside the iframe.

This means a download is a snapshot of the data the page held at that moment. Because `getData()`
uses the [response cache](#caching-nomis-api-responses-in-indexeddb), that may be a cached Nomis
response rather than a new request.

Embedded profiles are the exception. They are, in effect, persistently addressable, because all
their data is stored in the URL hash itself
([see above](#inlining-data-in-the-profile-url-hash)): anyone with the embed URL sees the same
profile, without the app needing to store anything. Downloads have no equivalent.
