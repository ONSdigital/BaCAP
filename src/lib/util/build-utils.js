import { centroids, version } from "$lib/stores/mapstore";
...
export function filterTopics(allTopics, level, coverage) {
	return allTopics
		...
		.filter((t) => !t.coverage || coverage.every((c) => t.coverage.includes(c)));
}
...
export async function getData(topic, comparison) {
	const compressedSelectionCodes = [
		...new Set([...get(buildstate).compressed.oa, ...get(buildstate).compressed.lsoa])
	].join("");
	const comparisonCodes = [
		...new Set([...(comparison?.codes?.oa ?? []), ...(comparison?.codes?.lsoa ?? [])])
	].join("");
...
export async function downloadData() {
	...
	download(file, `${bs.name ? bs.name.replaceAll(" ", "_") : "custom_area"}.csv`);
			...
			reader.onload = (e) => {
				try {
					...
					if (b) {
						...
						const comparison = {
							type: "place",
							areanm: props.areanm || props.name || props.areacd || "Comparison Area",
