<script>
	import { groupData } from "$lib/util/data";
	...
	let xMax = $derived(Math.max(...data.map((d) => d[xKey])));
	let zDomain = $derived(data.map((d) => d[zKey]).filter((v, i, a) => a.indexOf(v) === i));

	let xScale = $derived((value) => (value / xMax) * 100);
	let dataGrouped = $derived(groupData(data, yKey));
</script>
...
<div class="bar-chart" aria-hidden="true">
	...
	{#each dataGrouped as group}
		...
		<div class="bar-group" style:height="{barHeight}px">
			{#each group.values as d, i}
				{#if i == 0}
					<div class="bar" style:left="0" style:width="{xScale(d[xKey])}%"></div>
				{:else}
					<div
						class="marker"
